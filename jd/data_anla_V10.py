...
for k in ['好评','中评','差评']:
    ...
    filename = 'V10 wordcloud of {}'.format(k)
...
print('好评占比大于所有样本（好评率96.84%）的特征：')
print(features_option[(features_option['p_positive']>=0.9684)].index)
print('好评占比小于所有样本（好评率96.84%）的特征：')
print(features_option[(features_option['p_positive']<0.9684)].index)
indxs = list(features_option[(features_option['p_positive']<0.9684)].index)
add_para(prs, '好评占比小于所有样本（好评率96.84%）的特征', paragraph=' | '.join(indx for indx in indxs))
...
prs.save('report/京东华为荣耀V10评论分析报告.pptx')